<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <%@page import="com.oop.product.Product" %>
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ page import="com.oop.DBconnetion.DBconnection"%>
...
<nav id="navbar" class="navbar">
  ...
  ">Delete Account</button></a>
    ...
    <!-- End #main -->
		...
		<h1 class="fw-bold" style="margin-left:50px;">Your Listings</h1>
		
		
		
      		<!--<c:forEach var="product" items="${uploadedProducts}">
      			<div class="grid grid-cols-2 gap-1 p-2">

						<img class="p-2 rounded-lg" src="img/products/${product.getDisplayimage()}" alt="display image">
						<img class="p-2 rounded-lg" src="img/products/${product.getProductimage1()}" alt="display image 1">
						<img class="p-2 rounded-lg" src="img/products/${product.getProductimage2()}" alt="display image 2">
						
					</div>
					
					<div>
							<div class="flex flex-col justify-center items-center text-[#E9EBED] ">

								<p class="mb-3 font-normal capitalize ">
									<strong>title: </strong> ${product.getTitle()}

								</p>

								<p class="mb-3 font-normal capitalize ">
									<strong>status: </strong>${product.getAvailability()}</p>

								<p class="mb-3 font-normal capitalize ">
									<strong>category: </strong>${product.getCategory()}</p>
								<p class="mb-3 font-normal capitalize ">
									<strong>description: </strong>${product.getDescription()}</p>
								<p class="mb-3 font-normal capitalize ">
									<strong>model:</strong> ${product.getModel() }
								<p>
								<p class="mb-3 font-normal capitalize ">
									<strong>quantity: </strong>${product.getQuantity()}</p>
								<p class="mb-3 font-normal capitalize ">
									<strong>price:</strong> ${product.getPrice()}0

								</p>
								<p class="mb-3 font-normal capitalize ">
									<strong>posted Date: </strong>${product.getPostedDate()}
								</p>
								<p class="mb-3 font-normal capitalize ">
									<strong>availability:</strong> ${product.getAvailability()}
								</p>
      	...
      	<div style=	"display: flex; flex-wrap: wrap;">
			...
			try {
				...
				PreparedStatement pst = con.prepareStatement("SELECT * FROM product WHERE sellerid=?");
			...
			%>		
			   ...
			   <div class=" p-5" style="margin-left:30px;">
				   	<div class="text-center shadow-lg p-5" style="width:400px;border-radius:30px;">
				   	
					   	<div>
			               <img src="img/products/<%=rs.getString("displayimage")%>" style="width:150px;height:150px"><br>
			            </div>
			            
			            <div>
			               <img src="img/products/<%=rs.getString("productimage1")%>" style="width:150px;height:150px"><br>
			            </div>
			               
			            <img src="img/products/<%=rs.getString("productimage2")%>" style="width:150px;height:150px"><br><br>
			              
			               <form action="UpdateProductServlet" method="post">
			               	   <label class="text-left">Product Id:</label>
				               <input class="form-control" type="text" name="id" value="<%=rs.getInt("idproduct") %>" readonly><br>
				               ...
				               <input class="form-control" type="text" name="brand" value="<%=rs.getString("brand")%>"><br>
				               ...
				               <input class="form-control" type="text" name="model" value="<%=rs.getString("model")%>"><br>
				               
				               <label class="text-left">Quantity:</label>
				               <input class="form-control" type="text" name="qty" value="<%=rs.getInt("quantity") %>"><br>
				               
				               <label class="text-left">Price:</label>
				               <input class="form-control" type="text" name="price" value="<%=rs.getInt("price") %>.00"><br>
				               
				               <label class="text-left">Availability:</label>
				               <input class="form-control" type="text" name="availability" value="<%=rs.getString("availability") %>"><br>
